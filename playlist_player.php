    ...
    <div class="row">
      ...
      <div class="img-fond col-10">
        <div class="navbar bg-navbar">
          <button id="prev-navbar"><</button>
          <button id="next-navbar">></button>
        </div>

        <script>
          document.getElementById('prev-navbar').addEventListener('click', function() {
            history.back(); 
          });

          document.getElementById('next-navbar').addEventListener('click', function() {
            history.forward();
          });
        </script>
        ...
        <div class="pt-5">
          ...
          <div class="controls">
            <button id="prev-btn" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Précédent"><</button>
            <button id="play-btn" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Play/Pause">Play</button>
            <button id="next-btn" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Suivant">></button>
            <button id="loop-btn" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Lecture en boucle">Loop</button>
            <button id="random-btn" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Lecture aléatoire">Random</button>
          </div>
        </div>
        ...
        <script>
          ...
          prevBtn.addEventListener('click', function() {
            const currentIndex = Array.from(playButtons).findIndex(button => button.getAttribute('data-audio-id') === currentAudio.getAttribute('data-audio-id'));
          ...
          randomBtn.addEventListener('click', function() {
            // Vérifier si aucune musique n'est en cours de lecture
            if (currentAudio === null) {
              if (playButtons.length === 0) {
                // Aucune musique dans la playlist, rien à faire
                return;
              }

              // Sélectionner une musique aléatoire parmi toutes les musiques disponibles
              const musiquesCount = <?php echo count($musiques); ?>;
              const randomIndex = Math.floor(Math.random() * musiquesCount);
              const randomMusique = <?php echo json_encode($musiques); ?>[randomIndex];

              // Lancer la musique aléatoire sélectionnée
              const audio = new Audio("./audio/" + randomMusique['path']);
              audio.play();
              currentAudio = audio;
            } else {
              // Arrêter la musique en cours avant de lancer une nouvelle musique aléatoire
              stopAudio();
